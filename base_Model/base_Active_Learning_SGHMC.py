...
class base_Active_Learning_SGHMC(object):
    ...
    def _data_preprocess(self,target_dim=-1,fixed_seed=False,test_missing=0.2,**kwargs):
        rs = self.random_seed
        ...
        # Mask the train data
        train_mask, train_mask_other = base_generate_mask_incomplete(train_data, mask_prop=missing_prop)
        valid_mask,valid_mask_other =base_generate_mask_incomplete(valid_data,mask_prop=missing_prop)
    ...
    def train_BNN(self,observed_train,eps=0.01,max_sample_size=20,tot_epoch=100,thinning=50,hyper_param_update=200,sample_int=1,flag_hybrid=False,flag_reset_optim=True,W_dict_init=None,**kwargs):
        ...
        W_sample,_,_,_=train_SGHMC(self.Infer_model,observed_train,eps=eps,max_sample_size=max_sample_size,tot_epoch=tot_epoch,thinning=thinning,hyper_param_update=hyper_param_update,
                    sample_int=sample_int,flag_hybrid=flag_hybrid,flag_results=False,W_dict_init=W_dict_init,**kwargs
                    )
        return W_sample
        ...
        if selection_scheme=='overall':
            if not flag_initial:
                ...
                if Select_Split==True:
                    ...
                    idx_ob,flag_full,select_num_ob=BALD_Select_Explore(BALD_weight_ob,step_old)
                    ...
                    # Concat together
                    if not flag_full_un:
                        # concat old users and new user selection
                        idx_selected=torch.cat((idx_selected_unobserved,idx_selected_observed),dim=0)
                    else:
                        # if no new users, only selected old ones
